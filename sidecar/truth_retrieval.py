...
def merge_science_and_template_truth(
    ...
    template_truth_matched = (
        template_truth[matched_status].copy().reset_index(drop=True)
    )
    template_truth_unmatched = (
        template_truth[~matched_status].copy().reset_index(drop=True)
    )
    template_truth_unmatched["flux"] = template_truth_unmatched["flux"] * (-1)
    template_truth_unmatched["realized_flux"] = template_truth_unmatched[
        "realized_flux"
    ] * (-1)

    science_truth_new = science_truth.copy()

    science_truth_new.loc[matched_id[matched_status], "flux"] = (
        science_truth_new.loc[matched_id[matched_status], "flux"]
        - template_truth_matched.flux.to_numpy()
    )

    science_truth_new.loc[matched_id[matched_status], "realized_flux"] = (
        science_truth_new.loc[matched_id[matched_status], "realized_flux"]
        - template_truth_matched.realized_flux.to_numpy()
    )

    science_truth_new["image_type"] = ["science"] * len(science_truth_new)
    science_truth_new.loc[matched_id[matched_status], "image_type"] = [
        "both"
    ] * matched_status.sum()
    ...
    merged_truth = pd.concat([science_truth_new, template_truth_unmatched]).reset_index(
        drop=True
    )
