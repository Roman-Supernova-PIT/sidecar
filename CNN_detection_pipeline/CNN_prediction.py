...
def load_and_preprocess_fits(fits_path):
    ...
    try:
        ...
        print(f"Error loading file {fits_path}: {e!s}")
        return None, None
...
def create_snr_magnitude_analysis(results, output_folder):
    ...
    valid_results = [r for r in results['results']
                     if r['snr'] is not None and r['magnitude'] is not None]
    ...
    plt.subplot(2, 2, 1)
    plt.scatter(snrs, probabilities, c=predictions, cmap='coolwarm',
    ...
    plt.subplot(2, 2, 3)
    plt.scatter(magnitudes, probabilities, c=predictions, cmap='coolwarm',
                alpha=0.7, s=50, edgecolor='black')
    cbar = plt.colorbar(label='Prediction (1=Positive, 0=Negative)', ticks=[0, 1])
    cbar.ax.tick_params(labelsize=16)
    cbar.set_label('Prediction (1=Positive, 0=Negative)', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, 'snr_magnitude_analysis.png'),
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, 'detection_rate_analysis.png'),
                dpi=300, bbox_inches='tight')
    plt.close()
...
def create_summary_report(results, output_folder):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, 'processing_summary.png'),
                dpi=300, bbox_inches='tight')
    plt.close()

    summary_text = "Processing Summary (ZScale Normalized)\n"
    summary_text += "=" * 50 + "\n"
    ...
    summary_text += "Subfolder Breakdown:\n"
    summary_text += "-" * 50 + "\n"
...
def process_fits_folder_with_threshold(input_folder, output_folder, model, device,
                                        threshold=0.5):
    ...
    for fits_path in tqdm(fits_files, desc=f"Processing FITS files (threshold={threshold})"):
        ...
        prediction, probability = predict_single_image_with_threshold(model, image_tensor,
                                                                       device, threshold)
...
def save_image_with_prediction_threshold(image_data, prediction, probability, original_path,
                                          output_folder, metadata, threshold=0.5):
...
def main():
    ...
    print(f"\n{'='*70}")
    print("COMPLETE - All subdirectories processed")
    print(f"{'='*70}")
