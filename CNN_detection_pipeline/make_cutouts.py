...
def process_ecsv_and_create_cutouts(dia_out_dir='../dia_out_dir',
                                     cutout_size=64,
    ...
    for subdir_idx, subdir in enumerate(subdirs, 1):
        ...
        # Find ECSV files in this subdirectory - only process cleaned_score_detection_to_transients files
        ecsv_files = list(subdir.glob('cleaned_score_detection_to_transients_*.ecsv'))

        if not ecsv_files:
            print(f"No cleaned_score_detection_to_transients_*.ecsv files found in {subdir.name}")
            continue
