...
Date: Oct 14 2025
Prerequisite: Should have ecsv and fits files in each subdirectory of dia_out_dir.
what this code does: Makes cutouts (normalized) and then saves it in cutouts subfolder.
"""
...
def extract_cutout_with_padding(
    image_data, center_x, center_y, cutout_size=64, pad_value=0
):
    ...
    cutout = np.full(
        (cutout_size, cutout_size), pad_value, dtype=image_data.dtype
    )
...
def process_batch(
    ...
    for batch_idx, (idx, row) in enumerate(batch_data):
        try:
            ...
            if (x_center < -half_size or
                x_center >= image_data.shape[1] + half_size or
                y_center < -half_size or
                    y_center >= image_data.shape[0] + half_size):
                if debug_first and batch_idx < 3:
                    print("    ✗ Completely outside image!")
            ...
            if debug_first and batch_idx < 3:
                ...
                min_val = np.nanmin(cutout_raw)
                max_val = np.nanmax(cutout_raw)
                print(f"    Cutout range: [{min_val:.2f}, {max_val:.2f}]")
                non_zero = np.sum(cutout_raw != 0)
                print(f"    Non-zero pixels: {non_zero}/{cutout_raw.size}")
            ...
            base_filename = (
                f"cutout_{number:04d}_x{int(x_image)}_y{int(y_image)}"
            )
            ...
            raw_hdu.header['EDGECUT'] = (
                allow_edge_cutouts, 'Edge cutouts allowed'
            )
            ...
            fits_norm_file = (
                output_dirs['fits_norm'] / f"{base_filename}_norm.fits"
            )
...
def determine_fits_basename(ecsv_basename):
    ...
    Handles:
    - cleaned_score_detection_to_transients_XXX -> decorr_diff_XXX
    """
    if ecsv_basename.startswith('cleaned_score_detection_to_transients_'):
        return ecsv_basename.replace(
            'cleaned_score_detection_to_transients_', 'decorr_diff_'
        )
    else:
        return None


def process_ecsv_and_create_cutouts(
    ...
    """Process ECSV files and create cutouts from nested directory structure.
    ...
    edge_msg = (
        'Allowed (with padding)' if allow_edge_cutouts else 'Skipped'
    )
    print(f"Edge cutouts: {edge_msg}")
    print("=" * 70)
    ...
    for subdir_idx, subdir in enumerate(subdirs, 1):
        ...
        # Find ECSV files - only process cleaned_score_detection_to_transients
        ecsv_files = list(
            subdir.glob('cleaned_score_detection_to_transients_*.ecsv')
        )

        if not ecsv_files:
            msg = (
                f"No cleaned_score_detection_to_transients_*.ecsv files "
                f"found in {subdir.name}"
            )
            print(msg)
            continue
        ...
        for file_idx, ecsv_file in enumerate(ecsv_files, 1):
            ...
            try:
                ...
                for i in range(min(3, len(data))):
                    obj_id = (
                        data[i][id_col] if id_col in data.colnames else i
                    )
                    x_val = data[i][x_col]
                    y_val = data[i][y_col]
                    print(
                        f"    Object {obj_id}: "
                        f"{x_col}={x_val:.2f}, {y_col}={y_val:.2f}"
                    )
            ...
            if fits_basename is None:
                print("Skipping: ECSV file doesn't match expected pattern")
                continue
            ...
            # Create output directories inside the same subdirectory
            output_dir = subdir / 'cutouts'
            ...
            output_dirs = {
                'fits_raw': fits_raw_dir,
                'fits_norm': fits_norm_dir,
                'png': png_dir
            }
            ...
            if force_restart and checkpoint_file.exists():
                ...
                if processed_indices:
                    print(
                        f"Resuming: {len(processed_indices):,} "
                        f"already processed"
                    )
            ...
            remaining_indices = [
                i for i in range(len(data)) if i not in processed_indices
            ]
            total_batches = (
                (len(remaining_indices) + batch_size - 1) // batch_size
            )

            print(
                f"Processing {len(remaining_indices):,} objects "
                f"in {total_batches} batches"
            )

            for batch_idx in tqdm(
                range(0, len(remaining_indices), batch_size),
            ...
            print(f"\n{'─'*70}")
            print(
                f"Summary for {ecsv_file.name} ({catalog_type}):"
            )
            ...
            if successful_cutouts + skipped_cutouts > 0:
                total = successful_cutouts + skipped_cutouts
                success_rate = 100 * successful_cutouts / total
    ...
    if total_cutouts_created + total_skipped > 0:
        total = total_cutouts_created + total_skipped
        success_rate = 100 * total_cutouts_created / total
