#!/usr/bin/env python3
"""ECSV Cutout Generator.

User: BCN
Date: Oct 14 2025

Prerequisite: Should have ecsv/<tilename>.ecsv and fits folder should have
corresponding fits file.

What this code does: Makes cutouts (normalized) and then saves it in
cutouts/<tilename>/fits and png.
"""
    ...
    for batch_idx, (idx, row) in enumerate(batch_data):
        try:
            ...
            if debug_first and batch_idx < 3:
                ...
                print(f"    Cutout range: [{np.nanmin(cutout_raw):.2f}, "
                      f"{np.nanmax(cutout_raw):.2f}]")
...
def process_ecsv_and_create_cutouts(ecsv_dir='ecsv', fits_dir='fits',
    ...
    for file_idx, ecsv_file in enumerate(ecsv_files, 1):
        ...
        try:
            ...
            for i in range(min(3, len(data))):
                ...
                print(f"    Object {obj_id}: {x_col}={data[i][x_col]:.2f}, "
                      f"{y_col}={data[i][y_col]:.2f}")
        ...
        if successful_cutouts + skipped_cutouts > 0:
            success_rate = 100 * successful_cutouts / (successful_cutouts + skipped_cutouts)
    ...
    if total_cutouts_created + total_skipped > 0:
        overall_rate = 100 * total_cutouts_created / (total_cutouts_created + total_skipped)
